    ...
    <TkcSection className='space-y-8'>
      ...
      {isLoading && !data ? (
        ...
        <div className='space-y-8'>
          {/* Tab bar */}
          <div className='grid grid-cols-1 gap-2 sm:flex sm:gap-1.5 sm:overflow-x-auto sm:pb-1 sm:[-ms-overflow-style:none] sm:[scrollbar-width:none] sm:[&::-webkit-scrollbar]:hidden'>
            {tabs.map((tab) => (
              <button
                key={tab.key}
                type='button'
                ...
                className={cn(
                  'flex w-full items-center justify-between gap-2 rounded-xl border px-3 py-2 text-left text-[13px] font-semibold transition-all sm:w-auto sm:justify-start sm:gap-2 sm:rounded-full sm:px-5 sm:py-2.5 sm:text-sm sm:whitespace-nowrap',
                  activeTab === tab.key
